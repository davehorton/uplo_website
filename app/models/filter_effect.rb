module FilterEffect
  ...
  class Core
    ...
    def bias(val) 
            self.apply(){ |r, g, b, a|
                Pixel.new(
                    @filter.safe(r * bias_calc(r / 255, val)),
                    @filter.safe(g * bias_calc(g / 255, val)),
                    @filter.safe(b * bias_calc(b / 255, val)),
                    a        
                )
            }
            return self;
    ...
    def contrast(val) 
            self.apply(){ |r, g, b, a|
                Pixel.new(
                    @filter.safe(255 * contrast_calc(r / 255, val)),
                    @filter.safe(255 * contrast_calc(g / 255, val)),
                    @filter.safe(255 * contrast_calc(b / 255, val)),
                    a            
                )
            }
            return self;
    ...
    def posterize (levels) 
            step = (255 / levels).truncate;
            self.apply(){ |r, g, b, a|
                Pixel.new(
                    @filter.safe((r / step).truncate * step),
                    @filter.safe((g / step).truncate * step),
                    @filter.safe((b / step).truncate * step),
                    a
                )
            }
            return self;
    ...
    def tint(minRGB, maxRGB) 
            self.apply(){|r, g, b, a|
                Pixel.new(
                    @filter.safe((r - minRGB[0]) * ((255 / (maxRGB[0] - minRGB[0])))),
                    @filter.safe((g - minRGB[1]) * ((255 / (maxRGB[1] - minRGB[1])))),
                    @filter.safe((b - minRGB[2]) * ((255 / (maxRGB[2] - minRGB[2])))),
                    a
                )
            };
            return self;
...
#            }
            ...
            (0...@height).each do |i|
                (0...@width).each do |j|
                    ...
                    outDArray[outIndex]     = @filter.safe(r);
                    outDArray[outIndex + 1] = @filter.safe(g);
                    outDArray[outIndex + 2] = @filter.safe(b);
  ...
  class Blend 
    ...
    def apply(topFiltr, &block) 
      ...
      (0...@height).each do |i|
          (0...@width).each do |j|
                    index = (i*@width*4) + (j*4);
                    rgba = yield(
                        Pixel.new(blendDArray[index],
                         blendDArray[index + 1],
                         blendDArray[index + 2],
                         blendDArray[index + 3]),
                         Pixel.new(imageDArray[index],
                          imageDArray[index + 1],
                          imageDArray[index + 2],
                          imageDArray[index + 3])
                    );
    ...
    def multiply(topFiltr) 
            self.apply(topFiltr){|top, bottom|
                Pixel.new(
                    @filter.safe((top.r * bottom.r) / 255),
                    @filter.safe((top.g * bottom.g) / 255),
                    @filter.safe((top.b * bottom.b) / 255),
                    bottom.a 
                )
            }
            return self
