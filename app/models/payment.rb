...
class Payment
  ...
  def self.create_authorizenet_test(card_number, expiration, addresses)
    ...
    if addresses.has_key?(:address) and !addresses[:address].nil? and addresses[:address]!={}
      addresses[:address][:customer_address_id] = "address"
      address = AuthorizeNet::Address.new(addresses[:address])
      transaction.set_address(address)
    end

    if addresses.has_key?(:shipping) and !addresses[:shipping].nil? and addresses[:shipping]!={}
